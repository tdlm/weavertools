            ...
            <div className="mt-6 border-t border-gray-200">
                <dl className="divide-y divide-gray-200">
                    ...
                    {commonDiff.map((diff, index) => (
                        <div key={`common-${index}`} className="px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    {exclusiveToFirst.map((item, index) => (
                        <div key={`ex-first-${index}`} className="px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            <dt className="text-sm font-medium text-gray-900">{item.key}</dt>
                            <dd className="mt-1 text-sm text-red-500 sm:mt-0 sm:col-span-2 break-all">- {item?.value}</dd>
                        </div>
                    ))}

                    {/* Render items exclusive to the second array */}
                    {exclusiveToSecond.map((item, index) => (
                        <div key={`ex-second-${index}`} className="px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            <dt className="text-sm font-medium text-gray-900">{item.key}</dt>
                            <dd className="mt-1 text-sm text-green-500 sm:mt-0 sm:col-span-2 break-all">+ {item.value}</dd>
                        </div>
                    ))}
                </dl>
            </div>
